...
def main(pairs_fh, x1, x2, target, title, group):
  ...
  for r in csv.DictReader(pairs_fh, delimiter='\t'):
    ...
    nodes['{} {}'.format(X1, r[x1])] += 1
    nodes['{} {}'.format(X2, r[x2])] += 1
    if not group:
      ax.plot(("Proband", "Relative"), (jitter(int(r[x1]), Y_JITTER), jitter(int(r[x2]), Y_JITTER)), color=COLORS[int(r[x1])], marker='s', alpha=0.5, linewidth=4)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--log', action='store_true', help='more logging')
  ...
  main(sys.stdin, args.x1, args.x2, args.target, args.title, args.group)
